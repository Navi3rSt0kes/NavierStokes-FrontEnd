...
export default function ProductCard({ product }) {
  ...
  return <article className="group relative rounded-2xl border border-stone-200 bg-white p-3 transition duration-200 hover:-translate-y-1 hover:border-emerald-200 hover:shadow-lg hover:shadow-emerald-900/5">
    <button onClick={() => toggleFavorite(product)} aria-label="Guardar en favoritos" className={`absolute right-3 top-3 z-10 rounded-full bg-white p-2 shadow-sm ${saved ? 'text-rose-500' : 'text-slate-400 hover:text-rose-500'}`}><Heart size={17} fill={saved ? 'currentColor' : 'none'}/></button>
    ...
    <div className="mt-3 flex items-end justify-between gap-2"><div><p className="font-bold text-slate-900">{formatPrice(product.price)}</p>{product.oldPrice && <p className="text-xs text-slate-400 line-through">{formatPrice(product.oldPrice)}</p>}</div><button onClick={() => addToCart(product)} aria-label={`Agregar ${product.name}`} className="grid h-9 w-9 place-items-center rounded-xl bg-emerald-600 text-white transition hover:bg-emerald-700 active:scale-95"><Plus size={19}/></button></div>
